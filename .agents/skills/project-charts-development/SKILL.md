...
- GPU Line, Area, Stacked, and Stacked Bar backends are opt-in and resolve in the supported chain `WebGPU -> WebGL2 -> Dygraphs`. WebGPU shares one adapter/device and pipeline cache per SDK; WebGL2 shares one context/program cache per SDK. Capability, unsupported-visualization, initialization/pipeline/render, uncaptured device-error, device/context loss, and stale runtime failures must advance to the next supported backend without leaving a blank canvas. Evidence: `src/chartLibraries/{webgpu,webgl2}/`, `src/sdk/makeChart/filters/makeControllers.js`.
- The shared GPU Cartesian Line/Area/Stacked/Stacked Bar visualization owns one visible canvas for deterministic plot pixels and keeps semantic legend/toolbox/menu/popover surfaces in DOM. Area shares line payload/color buffers and draws one exact six-vertex baseline trapezoid per adjacent pair in reverse series order before straight/step strokes. Stacked packs row-major precision-normalized base/end bands from independent positive/negative totals, rebasing on visibility changes and resolving hover against signed bands. Stacked Bar reuses that residency and generates one series-major bar per finite value with the exact legacy width/fill/border contract. Pairs touching null gaps are discarded and null bars are omitted. WebGPU draws directly; WebGL2 renders on the shared context and copies the completed exact frame to the visible chart canvas to avoid per-chart context limits. Browser-shaped complete strings use shared Canvas2D shaping and backend-owned bounded atlases. Evidence: `src/chartLibraries/gpu/`, `src/chartLibraries/{webgpu,webgl2}/text/`.
...
- `src/chartLibraries/gpu/`: backend-neutral exact Line/Area/Stacked/Stacked Bar visualizations, payload/range indexing, axes, overlays, interactions, color, and text shaping.
