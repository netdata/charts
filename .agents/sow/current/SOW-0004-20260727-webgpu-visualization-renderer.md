...
Sub-state: The production-capable opt-in Line, Area, Stacked, Stacked Bar, Multi Column, Heatmap, and EasyPie/Circle renderers pass exact physical WebGPU/WebGL2 adapter correctness, parity, fallback, lifecycle, and export gates on the clean integration branch from current `origin/main`. All Cartesian families and the first radial family in the approved migration are implemented. Gauge is next, followed by D3 Pie before the final cross-platform matrix. Runtime/power policy and rollout/defaulting remain deferred, and all legacy renderers remain every default.
...
- Reworked the headed benchmark to reuse one Chromium context with at most one live benchmark page. Cases reload that page, and backend groups replace it only to release accumulated GPU state. This preserves isolation while preventing repeated focus-stealing windows; temporary CDP sessions are detached after each measured case.
...
- The combined all-renderer run was intentionally not accepted as a new Line performance baseline: active-workstation contention caused previously proven Cartesian gates to vary, and repeated generic WebGL2 parity captures became blank after accumulated shared-page GPU state. EasyPie correctness and fallback passed independently. The harness now periodically replaces its sole page within the same Chromium window to bound accumulated GPU state; final platform validation will rerun the complete suite on an idle workstation.
...
The production-capable Line, Area, Multi Column, diverging Stacked, Stacked Bar, Heatmap, and EasyPie/Circle GPU adapters are implemented and physically validated on the clean integration branch. The approved next milestone is Gauge, followed by D3 Pie before broader browser/device hardening. Runtime/power policy and rollout/default approval remain intentionally deferred.
