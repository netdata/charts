...
Sub-state: The production-capable opt-in Line, Area, Stacked, Stacked Bar, Multi Column, Heatmap, EasyPie/Circle, Gauge, and D3 Pie renderers pass physical WebGPU/WebGL2 adapter correctness, parity, fallback, lifecycle, and export gates on the clean integration branch from current `origin/main`. The approved graphical-family migration is complete. Maintainer-readiness hardening is now active: the public `chartLibrary` mutation and required Cloud renderer-aware dispatch were identified as an abstraction leak and must be replaced by fully internal backend selection/fallback before the PR is shared. Runtime/power policy and rollout/defaulting remain deferred, and all legacy renderers remain every default.
...
- Rendering backend selection and fallback are private implementation details. Public `chartLibrary` and React component identity remain stable through `WebGPU -> WebGL2 -> legacy`; consumers must not dispatch, subscribe, or change behavior based on backend names. The installed integration must work against unmodified Cloud Frontend source.
...
- Cloud Frontend dispatches React components from the established public `chartLibrary` identity. The current implementation writes `webgpu` or `webgl2` into that public attribute and therefore requires Cloud to understand internal backends; this is an abstraction leak. The renderer controller must keep public identity stable and replace only the package-owned chart UI.
...
2. Refactor the internal selection model so visualization identity is independent of active renderer backend. Keep public `chartLibrary`, consumer component selection, payloads, events, and existing defaults unchanged; store requested/active backend and fallback state privately, and replace only the package-owned chart UI.
...
- `netdata/cloud-frontend @ bf2ba8182cff`:
  - `src/charts/index.js`: current public `chartLibrary` component dispatch is a compatibility contract. GPU backend selection must remain inside Charts so this consumer requires no renderer-aware source change.
...
19. **Heatmap adapter:** approved and implemented under the existing ordered migration. Pack row-major cell values from public `payload.all` rows using the same absolute/default or cumulative-to-incremental bucket semantics as `getRowDimensionValue`, without mutating payloads. Preserve numerically sorted/cropped bucket IDs, visible-rank reflow, first-pair reduced-window cell width, one centered cell per source value, null/zero transparency, negative incremental values, the existing color-scale behavior, transparent stroke, categorical bucket labels/grid, and y-nearest bucket hover. Reuse shared x residency, Cartesian x-axis/overlays/interactions/text/lifecycle, but use small backend-specific Heatmap shaders and four-vertex triangle strips because the generic bar shader missed the strict 100,000-value mount-presentation gate. Do not reinterpret cell values as y coordinates or add line gap markers.
20. **EasyPie/Circle adapter:** approved and implemented under the existing ordered migration. Preserve the current Easy Pie Canvas geometry and value semantics: sum the selected current/hover row exactly as today; normalize against `getValueRange`; clamp drawing to signed `[-100%, 100%]`; retain clockwise positive and counter-clockwise negative progress from 12 o'clock; preserve round progress caps, full track, 24 alternating scale ticks, size/line-width/scale-length formulas, theme colors, y-axis notifications, and immediate GPU resizing. Keep the existing React value/unit text in DOM. Use one small analytic radial shader per backend with exact physical-pixel coverage and source-over composition; no payload residency, sampling, local pointer interaction, animation, or generic speculative radial framework is needed.
21. **Gauge adapter:** approved and implemented under the existing ordered migration. Preserve current/hover row summation, range notification, the clamped `[0.001%, 99.999%]` display value, the legacy 252-degree arc and sizing formulas, flat or conic-gradient progress, threshold-selected whole-progress color, remainder track, and immediate pointer triangle/circle geometry. Keep value, unit, and bounds labels in React DOM. Render arc and pointer analytically in one backend-specific draw. `staticZones` is uncommon legacy configuration with arbitrary zone geometry; until exact zone storage is implemented, detect it and fall through the normal renderer chain rather than ignoring or approximating it.
22. **D3 Pie adapter:** approved and implemented. Preserve current/hover row selection, absolute wedge size with signed label values, label-ascending ordering, exact top-five small-segment grouping, colors, donut geometry, one-pixel segment stroke, no-data state, y-range notification, hover darkening, and click pull-out. GPU owns only deterministic wedge pixels. Keep the existing SVG label/connector layout and invisible interactive wedge hit targets as a semantic overlay so collision behavior, text formatting, hover targets, and click timing are not approximated or duplicated.
23. **Renderer encapsulation and same-backend visualization changes:** approved and required before maintainer review. Public `chartLibrary` and React component identity remain stable while private backend state changes through `WebGPU -> WebGL2 -> legacy`; consumers must not know backend names, subscribe to backend replacement, or alter dispatch. Charts must replace the package-owned chart UI on backend or visualization changes, including same-backend transitions such as Gauge to D3 Pie, while ownership-safe container cleanup prevents stale unmounts. The isolated Cloud renderer-aware dispatch change is superseded and must be reversed; acceptance requires unmodified Cloud Frontend source. Sparkline dimension-color pairs still resolve to the active theme string before any renderer consumes them.
24. **Shared GLSL uniform arity remains exact:** Heatmap extends its fill state to four values, but the shared non-Heatmap shader still declares `uFill` as `vec3`. Non-Heatmap uploads must call `uniform3f(x, y, z)` rather than passing the four-value backing array to `uniform3fv`; WebGL rejects an array whose length is not a multiple of the declared vector arity.

## Maintainer-Readiness Hardening Checklist

All items below remain inside the single integrated Charts PR. The PR stays directly installable and testable as one proof; maintainers will decide any eventual merge split after reviewing the complete implementation.

1. Keep renderer backend selection and fallback fully internal. Preserve public `chartLibrary` and React component identity, remove required consumer renderer awareness, reverse the isolated Cloud dispatch adaptation, and prove the complete fallback chain with unmodified Cloud Frontend source.
2. Add blocking pull-request CI for Jest, repository-configured ESLint, CJS/ES6 builds, Storybook, and deterministic real-browser GPU correctness. Keep hardware performance gates separate from shared-runner correctness.
3. Pin the browser harness dependency and make its Chromium requirements reproducible.
4. Remove dead/broken feasibility code and unreleased compatibility wrappers that have no consumer.
5. Extract renderer routing/state from filter controllers and establish one canonical visualization metadata registry without per-chart map allocation or duplicated lists.
6. Consolidate genuinely identical WebGPU/WebGL2 renderer lifecycle behavior while keeping backend-specific resource and loss behavior explicit.
7. Split shared shader source by concern, centralize WebGL program ownership, replace positional uniform arrays with named backend packers, and test declared uniform type/arity.
8. Add real-browser coverage for initialization/unmount races, every shader and visualization, export, unsupported configurations, same-backend adapter replacement, and the full loss/fallback chain.
9. Split the monolithic benchmark runner into explicit browser/session, workload, Cartesian parity, radial parity, fallback, and reporting modules; remove its fake Playwright context facade while retaining exactly one real context/window/page.
10. Share the WebGL2 text atlas through the runtime, define ownership/retirement, and make resource diagnostics account for shared textures rather than reporting only dynamic buffers.
11. Add concise architecture, extension, fallback, ownership, and validation documentation for maintainers without exposing backend details as a consumer requirement.
12. Add a deterministic Storybook renderer gallery covering both accelerated backends, every implemented visualization, and forced fallback states.
13. Add optional internal diagnostics for requested/active backend, backend health, adapter/context information, fallback reason, and resource use; normal callers must never need these diagnostics for rendering.
14. Keep the work as one coherent PR for proof, installation, and maintainer review. Organize the commit sequence and review guide by subsystem, and remove internal process-only noise from the eventual public diff.
15. Re-run the complete source, physical Linux, installed unmodified-Cloud, export, lifecycle, and authorized cross-platform validation after structural hardening.
...
5. Complete Stacked, Stacked Bar, Multi Column, Heatmap, EasyPie/Circle, Gauge, and D3 Pie in order, applying the same adapter-specific parity gate to each. Completed.
...
- Verified that Cloud Frontend's `src/charts/index.js` recognizes only `dygraph` as a line renderer for non-sparkline charts. The initial response was to dispatch through `chart.isTimeSeriesRenderer()`; this was superseded on 2026-07-30 because backend-aware consumer dispatch violates renderer encapsulation.
...
- Initial Cloud Frontend validation used the duplicate checkout at `$HOME/src/netdata/cloud-frontend`; that evidence was invalid. Rechecking established `$HOME/src/dashboard/cloud-frontend` as the active shared checkout, but touching it would conflict with another worker. All charts, Cloud Frontend, and netdata-ui work is now isolated under `$HOME/src/PRs/webgpu-charts/`.
...
- Implemented Heatmap over public `payload.all` residency with exact default absolute and cumulative-to-incremental conversion, row-major cells, stable payload caching, numeric bucket sorting/cropping, visibility rank reflow, categorical axes, nearest-row hover, reduced-window width, and cell-value y-axis notifications. Null and zero cells remain transparent; negative incremental differences remain values rather than being mistaken for gaps.
- The first shared-bar shader path preserved pixels but was borderline at the strict 100,000-cell mount gate. Added backend-specific Heatmap shaders and Heatmap-only four-vertex triangle strips, retaining exact Canvas2D edge coverage while removing unused base/stroke/stacking branches without changing Stacked Bar or Multi Column kernels.
- Deterministic probes matched Dygraphs exactly on both backends: top `[252,238,0,255]`, transparent middle `[0,0,0,0]`, and bottom `[42,171,129,255]`; horizontal and vertical footprint deltas were zero, categorical range matched `[-0.09,3.09]`, injected null cells produced a fully transparent gap band, and line step mode was a no-op.
- Physical Chromium 150/X11 WebGL2 passed the complete benchmark. At 100,000 values mount/update work completed in 15.3/5.4 ms and presented in 16.9/16.7 ms; at 1,000,000 values frame-settled mount/update gains were 61.39x/67.74x. Exports, four-chart lifecycle, all prior adapters, and exact parity passed. Evidence: `/tmp/gpu-heatmap-webgl2-specialized.clean.json`.
- Physical Chromium 150/Wayland WebGPU passed the Heatmap 100,000-value strict gate once at 15.6 ms mount work/15.7 ms presentation and 9.1/16.6 ms update, with 53.73x/43.04x frame-settled 1,000,000-value gains. Repeated runs under active-workstation contention varied: the final correctness run passed every adapter, parity, export, lifecycle, fallback, 100,000 update, and 1,000,000 gates but missed initial mount at 18.1/32.1 ms. Both results are retained rather than hiding the variance. Evidence: `/tmp/gpu-heatmap-webgpu-specialized.clean.json` and `/tmp/gpu-heatmap-webgpu-specialized-final.clean.json`.
- Reworked the headed benchmark to reuse one Chromium context, one window, and the same benchmark page for the complete suite. Cases reset through cleanup and navigation; no page is closed or replaced between cases. Temporary CDP sessions are detached after each measured case.
- Implemented EasyPie/Circle as the first radial GPU adapter. The backend-neutral model preserves latest/synchronized-hover row selection, all-dimension summation, signed range normalization, degenerate-range behavior, exact size/line-width/scale-length/radius formulas, theme colors, and y-axis notifications. Existing React value/unit text remains semantic DOM.
- Added dedicated analytic WebGPU and WebGL2 radial shaders. One four-vertex draw composes the 24 one-pixel radial scale ticks, full track ring, and signed round-capped progress arc in physical pixels. Positive progress remains clockwise from 12 o'clock; negative progress remains counter-clockwise; both clamp at one revolution.
- Physical Chromium 150/Wayland probes matched the Easy Pie Canvas baseline exactly at every sampled positive and negative interior/track/center pixel. WebGPU and WebGL2 each had zero RGBA delta, zero horizontal ring-width delta in the hardware run, and only 111 additional antialiased edge pixels out of 250,000. A headless WebGL2 rerun retained zero sampled RGBA delta, one-pixel footprint delta, and 307 edge-pixel delta.
- Forced EasyPie loss routing passed `WebGPU -> WebGL2 -> easypiechart`, with no Dygraphs instance and zero retained WebGL2 contexts. Evidence: `/tmp/gpu-easy-pie-combined-third.clean.json`, `/tmp/gpu-easy-pie-combined-final.clean.json`, and `/tmp/gpu-easy-pie-webgl2-headless-reset.clean.json`.
- The combined all-renderer run was intentionally not accepted as a new Line performance baseline: active-workstation contention caused previously proven Cartesian gates to vary. EasyPie correctness and fallback passed independently. The harness now keeps one page for the complete headed suite and resets it by navigation; final platform validation will rerun the complete suite on an idle workstation.
- Implemented Gauge with backend-neutral current/hover summation, exact range notification, `[0.001%, 99.999%]` clamping, legacy 252-degree sizing, smooth-gradient or threshold-selected progress, remainder track, and pointer triangle/circle geometry. React retains value/unit/min/max labels. Unsupported `staticZones` throws into the normal fallback chain rather than being ignored or approximated.
- Physical Chromium 150/Wayland Gauge probes passed on both backends. Progress, track, pointer, and empty samples differed from Canvas2D by at most four RGBA levels; the complete footprint differed by 60 edge pixels out of 250,000. Forced loss routing passed `WebGPU -> WebGL2 -> gauge` with zero retained WebGL2 contexts. Evidence: `/tmp/gpu-gauge-radial-single-page.clean.json` and `/tmp/gpu-radial-all-single-page-final.clean.json`.
- Implemented D3 Pie with exact current/hover values, absolute wedges plus signed labels, label-ascending order, top-five grouping, no-data behavior, donut sizing, one-pixel stroke, hover darkening, and five-pixel click pull-out. GPU owns wedge pixels; the existing SVG label/connector layout and invisible wedge hit targets remain as a semantic overlay, preserving text collision layout and interaction timing without approximation.
- Physical Chromium 150 validation passed D3 Pie on hardware WebGPU and WebGL2: all six interior/center/outside samples had zero RGBA delta, hover darkening had zero delta, the raster footprint differed by 162 edge pixels, all six labels/connectors matched, and the pull-out offset was exactly five physical pixels. Both fallback chains reached legacy `d3pie` with zero retained WebGL2 contexts. Evidence: `/tmp/gpu-radial-webgpu-single-page-final.clean.json`, `/tmp/gpu-radial-all-single-page-final.clean.json`, and `/tmp/gpu-d3pie-headless-fixed.clean.json`.
- Final source validation after Gauge and D3 Pie passed 185 Jest suites with 1,641 tests passed and 2 skipped; coverage remained above policy at 61.27% statements, 55.54% branches, 58.99% functions, and 62.47% lines. Changed-file ESLint, CJS/ES6 builds with 620 files each, Storybook production build, SOW audit, and `git diff --check` passed. Storybook reported only its existing module-type, missing-MDX-pattern, and bundle-size warnings.
- Installed-Cloud smoke testing found two integration defects that isolated renderer tests could not expose. First, changing visualization while retaining `webgl2` kept the old adapter because UI replacement was keyed only by renderer name and Cloud did not subscribe to `chartUIChanged`. Second, sparkline `colors` can contain light/dark pairs, but `selectDimensionColor()` returned the whole pair; Gauge's gradient correctly rejected that non-string input.
- Replaced adapters on visualization changes even when the renderer is unchanged, made ChartContainer cleanup ownership-safe, subscribed Cloud dispatch to the current chart UI and visualization, and resolved sparkline pairs through the active theme index. Real lifecycle tests cover same-renderer Gauge-to-D3-Pie replacement and moved-container cleanup; color tests cover paired sparkline colors. The Cloud subscription/dispatch change was later rejected as an abstraction leak and is superseded by the 2026-07-30 decision; ownership-safe internal adapter replacement and theme color repair remain valid.
- The rebuilt installed Cloud dashboard passed a one-page headless consumer smoke with no console/page errors. One stable live chart changed sequentially through EasyPie, Gauge, and D3 Pie while retaining WebGL2; each current adapter and visible canvas remained connected, and D3 Pie retained six SVG labels. Adding `staticZones` selected legacy Gauge without disabling WebGL2 for other visualizations; removing it restored WebGL2 Gauge. The installed Charts bundle exactly matches the final Cloud build at `sha256 a7fcb0a79c1548ae39a780c2581bbf12ba4e2aa8864d89bb9ddac7cc04ece1bd`.
- Post-integration source validation passed 185 Jest suites with 1,644 tests passed and 2 skipped; coverage remained above policy at 61.29% statements, 55.58% branches, 59.02% functions, and 62.50% lines. Changed-file ESLint, CJS/ES6 builds with 620 files each, Cloud production build, SOW audit, and `git diff --check` passed.
- The final combined Linux run exposed a real shared-WebGL fill regression: Heatmap added a fourth fill-state value, while Area, Stacked, Stacked Bar, and Multi Column continued to declare `uFill` as `vec3`. Passing the four-value array to `uniform3fv` produced `INVALID_VALUE`; strokes and specialized Heatmap/radial shaders still rendered, but generic fills retained zero uniforms. The kernel now uploads the three declared values explicitly with `uniform3f`.
- A focused real-browser Area capture immediately restored the exact Dygraphs pixels `[255,0,0,51]` and `[141,0,114,92]`. The final physical Chromium 150/Wayland WebGL2 run passed Line plus every Area, Heatmap, Multi Column, Stacked, Stacked Bar, EasyPie, Gauge, and D3 Pie correctness/parity check. The strict 100,000-value gate passed at 15.1 ms mount work/16.5 ms presentation and 4.5 ms update work/16.6 ms presentation. The active-workstation 1,000,000-value run reached only 4.07x/3.61x frame speedups and is retained as contention evidence, not accepted as a replacement for prior greater-than-5x baselines. Evidence: `/tmp/gpu-all-webgl2-hardware-fill-fixed.clean.json`.

### 2026-07-30

- The maintainer-readiness review found that writing `webgpu`/`webgl2` into public `chartLibrary` forced Cloud Frontend to understand and dispatch on internal renderer state. The user rejected that contract: rendering backend selection and fallback are purely internal, public component identity must stay stable, and unmodified Cloud Frontend must continue to work.
- Added the complete maintainer-readiness hardening checklist. The work remains one integrated PR during proof and maintainer installation/testing; merge splitting is a later maintainer decision, not a pre-review requirement.
- Replaced public backend routing with a private renderer controller and canonical visualization metadata. Public `chartLibrary` now stays `dygraph`, `gauge`, `d3pie`, or `easypiechart` while private state moves through accelerated and legacy implementations. Removed the backend-oriented consumer methods/maps, added optional diagnostics, preserved same-backend visualization replacement, and passed the full 191-suite Jest run after the refactor.
- Reversed the isolated Cloud renderer-aware source and skill changes in local Cloud commit `3a7fe9586`; those files now exactly match `origin/develop`. The remaining integration acceptance test is an accelerated build installed against that unmodified consumer source.
- Consolidated shared renderer lifecycle, resource-set failure cleanup, runtime resource caching, visualization registries, and radial resource composition. WebGL2 now shares one runtime-owned 4 MiB text atlas across mounted charts and reports it separately from per-chart buffers.
- Split the shared GLSL source by concern, centralized shared-program acquisition, and added typed uniform writers plus named WebGPU/WebGL2 frame packers with exact arity tests. The Cartesian neutral frame renderer is separated from mount/interaction ownership.
...
- Full Jest with coverage: 182 suites passed; 1,629 tests passed and 2 skipped. Coverage passed unchanged thresholds at 62.30% statements, 55.99% branches, 60.17% functions, and 63.52% lines.
- Focused shared-GPU, WebGPU, WebGL2 primitive, routing, controller, hover, and default-SDK tests passed without new Jest mocks.
- Clean CommonJS and ES6 distributions built with 601 files each; moved backend-neutral modules and both GPU backends were present, while stale pre-move paths were absent.
...
- Physical Line, Area, Multi Column, Stacked, Stacked Bar, and Heatmap benchmarks passed exact source geometry/gaps, Dygraphs pixels, PNG export, four-chart shared-runtime lifecycle, and the complete runtime-loss chain. Every 1,000,000-value and 100,000-value update gate passed; WebGL2 passed the 100,000-value mount gate consistently, while WebGPU Heatmap mount passed once and remained contention-sensitive as recorded in the execution log.
...
The production-capable Line, Area, Multi Column, diverging Stacked, Stacked Bar, Heatmap, EasyPie/Circle, Gauge, and D3 Pie GPU adapters are implemented and physically validated on the clean integration branch. The approved graphical-family migration is complete. Storybook/Cloud consumption and broader browser/device hardening are next; runtime/power policy and rollout/default approval remain intentionally deferred.
