...
23. **Same-backend visualization changes:** changing visualization identity must replace the chart UI adapter even when the backend name is unchanged, such as `Gauge/webgl2 -> D3 Pie/webgl2`. Consumers must subscribe to `chartUIChanged` and include visualization identity in component dispatch. Stale container cleanup may unmount a UI only when it still owns the same DOM node. Sparkline dimension-color pairs must resolve to the active theme string before either legacy or GPU renderers consume them.
