...
Sub-state: The production-capable opt-in Line, Area, Stacked, Stacked Bar, Multi Column, and Heatmap renderers pass exact physical WebGPU/WebGL2 correctness, parity, fallback, lifecycle, export, and high-cardinality gates on the clean integration branch from current `origin/main`. All Cartesian families in the approved migration are implemented. EasyPie/Circle is next, followed by Gauge and D3 Pie before the final cross-platform matrix. Runtime/power policy and rollout/defaulting remain deferred, and Dygraphs remains every default.
...
- Reworked the headed benchmark to reuse one Chromium context and one page for every workload and correctness case. This preserves page-reload isolation while preventing repeated focus-stealing windows; temporary CDP sessions are detached after each measured case.
...
- Full Jest with coverage: 181 suites passed; 1,624 tests passed and 2 skipped. Coverage passed unchanged thresholds at 62.82% statements, 56.13% branches, 60.70% functions, and 64.07% lines.
- Focused shared-GPU, WebGPU, WebGL2 primitive, routing, controller, hover, and default-SDK tests passed without new Jest mocks.
- Clean CommonJS and ES6 distributions built with 593 files each; moved backend-neutral modules and both GPU backends were present, while stale pre-move paths were absent.
...
The production-capable Line, Area, Multi Column, diverging Stacked, Stacked Bar, and Heatmap GPU adapters are implemented, physically validated, and cleanly integrated on current `origin/main`. The approved next milestone is EasyPie/Circle, followed by Gauge and D3 Pie before broader browser/device hardening. Runtime/power policy and rollout/default approval remain intentionally deferred.
