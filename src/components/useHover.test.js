...
describe("useHover", () => {
  ...
  it("adds event listeners when ref.current is set", () => {
    const { result, rerender } = renderHook(() => useHover({ onHover: mockOnHover, onBlur: mockOnBlur }, []))
  ...
  it("calls onHover when mouseover event is triggered", () => {
    ...
    const mouseoverHandler = mockElement.addEventListener.mock.calls.find(call => call[0] === "mouseover")[1]
  ...
  it("calls onBlur when mouseout event is triggered and mouse leaves element", () => {
    ...
    const mouseoutHandler = mockElement.addEventListener.mock.calls.find(call => call[0] === "mouseout")[1]
  ...
  it("removes event listeners on unmount", () => {
    const { result, unmount, rerender } = renderHook(() => useHover({ onHover: mockOnHover, onBlur: mockOnBlur }, []))
  ...
  it("handles dependencies correctly by re-running effect", () => {
    let deps = ["dep1"]
    const { rerender } = renderHook(() => useHover({ onHover: mockOnHover, onBlur: mockOnBlur }, deps))
...
describe("useHovered", () => {
  ...
  it("sets focused to false on blur", () => {
    ...
    const onMouseout = mockElement.addEventListener.mock.calls.find(call => call[0] === "mouseout")[1]
    act(() => {
