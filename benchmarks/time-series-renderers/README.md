...
This task-specific comparator measures Dygraphs and selected production GPU backends from the same checkout with identical deterministic Line, Area, diverging Stacked, or Stacked Bar data. WebGPU is preferred; WebGL2 is the accelerated compatibility fallback.
...
Measure Area, Stacked, or Stacked Bar against Dygraphs while retaining all implemented visualization correctness checks:
...
Both GPU backends use the same production visualization/data/interaction model, including precision-normalized values, exact null gaps, line step/smooth geometry, Area baseline trapezoids, diverging Stacked base/end bands, Stacked Bar rectangles, axes, text, overlays, and interactions. Filled-line correctness requires one exact band per adjacent source pair, a fully empty null-gap band, and distinct regular/step pixels. Stacked Bar requires one instance per source value, exact null omission, and no response to line step mode. Pixel probes require Dygraphs RGBA parity for Area reverse overlap/baseline behavior, Stacked reverse-order positive/negative bands, and Stacked Bar range, width, fill, subpixel border, and empty pixels. WebGL2 owns only its GLSL shaders, textures/buffers, shared context/program runtime, presentation surface, and context-loss handling.
...
- Geometry: every visible series and adjacent pair; Area and Stacked add one exact fill band per pair, while Stacked Bar adds one exact rectangle per source value; no LOD, sampling, or aggregation.
